...
MYSQL_ERROR_LOG_DIR="${MYSQL_ERROR_LOG_DIR:-${DATADIR}}"
wsrep_notify_cmd=${WRESP_NOTIFY_CMD:-/docker-scripts.d/wsrep_notify.sh}

if [ -f "${DATADIR}/master_rep.info" ];then
    master_server_id=$(cat ${DATADIR}/master_rep.info | grep master_server_id | cut -d\= -f2)
  else
...
if [ -z "${SERVER_NAME}" ];then
  SERVER_NAME="${CLUSTER_NAME:-pxc-cluster}"
fi
...
func_consul_agent_start(){
  ...
  echo "{ \"services\": [ { \"name\": \"${CONSUL_SERVICE}\", \"address\": \"$ipaddr\", \"port\": 3306, \"Tags\": $tags, \"checks\": [ { \"name\": \"check if percona port is open\", \"tcp\": \"$ipaddr:3306\", \"interval\": \"35s\", \"timeout\": \"5s\" } ] } ] }" > /tmp/pxc.json
...
# Starting consul client
  ...
  if [[ $(ps aux | grep "consul.*agent" | grep -ve 'grep') == "" ]];then
      ...
      #/bin/consul agent -retry-join $DISCOVERY_SERVICE -client 0.0.0.0 -bind $ipaddr -node $SERVER_NAME-$hostname -data-dir /tmp -config-file /tmp/pxc.json &>/dev/null & disown;
      echo "Sleep for ${CONSUL_AGENT_SLEEP_TIME} seconds after agent start" && progress-bar ${CONSUL_AGENT_SLEEP_TIME}
  else
...
mysql_tune(){
  ...
  if [[ "${MYSQL_TUNE_BEFORE_START}" == "true" ]];then
      if [[ -f "${m_config}" ]];then
        ...
        grep -q '^wsrep_slave_threads' "${m_config}" && sed -i  "s/wsrep_slave_threads.*$/wsrep_slave_threads = ${WSREP_SLAVE_THREADS:-2}/g" "${m_config}" || echo "wsrep_slave_threads = ${WSREP_SLAVE_THREADS:-2}" >> "${m_config}"
      ...
      echo "File ${MYSQL_CONF_FILE:-/etc/mysql/conf.d/node.cnf} not exist. Enable tuning"
    fi
  fi
  if [[ "${GTID_REPLICATION}" == "true"  && "${REPLICATION_ROLE}" == "master" ]];then
    ...
    if [[ -f "${m_config}" ]];then
      ...
      echo "File ${MYSQL_CONF_FILE:-/etc/mysql/conf.d/node.cnf} not exist. Skip enable replication"
    fi
  fi
  echo "End func ${FUNCNAME[0]}"
  if [[ ! -f "${DATADIR}/master_rep.info" ]];then
    echo "master_server_id=${hostname//[!0-9]/}" > ${DATADIR}/master_rep.info
  fi
}
...
mysql_main(){
  if [ -z "$CLUSTER_JOIN" ]; then
    if [ ! -e "${DATADIR}/mysql" ]; then
       ...
       echo "Exec mkdir -pv ${DATADIR}"
       mkdir -pv "${DATADIR}"
       chown -R mysql:mysql ${DATADIR} || echo "error chown -R mysql:mysql ${DATADIR} "
       ...
       if [ -z "$EXPORTER_PASSWORD" ]; then
        EXPORTER_PASSWORD="$(pwmake 128)"
        echo "GENERATED exporter user PASSWORD: $EXPORTER_PASSWORD"
        echo "User exporter Password $EXPORTER_PASSWORD" >> ${DATADIR}/exporter.info
       fi
...
"${mysql[@]}" <<-EOSQL
        ...
        GRANT USAGE ON *.* TO 'hpclustercheckuser'@'%' WITH MAX_CONNECTIONS_PER_HOUR 0;
        FLUSH PRIVILEGES ;
EOSQL
...
set -e
    ...
    ln -sf  ${MYSQL_ERROR_LOG_DIR}/error.log /dev/stdout
    chown mysql:mysql ${MYSQL_ERROR_LOG_DIR}/error.log
  /bin/cat <<OEF> /etc/supervisor/conf.d/pxc-5-7.conf

[supervisord]
nodaemon=true

[program:pxc-5-7]
command=mysqld --user=mysql  --log-error-verbosity=2 --wsrep_cluster_name=$CLUSTER_NAME --wsrep_cluster_address="gcomm://$cluster_join" --wsrep_sst_method=xtrabackup-v2 --wsrep_sst_auth="xtrabackup:$XTRABACKUP_PASSWORD" --wsrep_node_address="$ipaddr"  --log-error=${MYSQL_ERROR_LOG_DIR}/error.log $CMDARG
...
echo "------------------------"
echo "mysqld --user=mysql  --log-error-verbosity=2 --wsrep_cluster_name=$CLUSTER_NAME --wsrep_cluster_address=\"gcomm://$cluster_join\" --wsrep_sst_method=xtrabackup-v2 --wsrep_sst_auth=\"xtrabackup:$XTRABACKUP_PASSWORD\" --wsrep_node_address=\"$ipaddr\"  --log-error=${MYSQL_ERROR_LOG_DIR}/error.log $CMDARG"
echo "------------------------"
}
